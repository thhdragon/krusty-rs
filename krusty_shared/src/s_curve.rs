// src/s_curve.rs - Shared S-curve motion profile generator
use thiserror::Error;
use std::sync::{Arc, Mutex};
use std::time::Duration;
use crate::event_queue::{SimEventQueue, SimClock, SimEvent, SimEventType};
use crate::StepCommand;
...
impl SCurveGenerator {
    ...
    pub fn generate_and_schedule_s_curve(
        ...
        cruise_velocity: f64,
        axis: usize,
        event_queue: &Arc<Mutex<SimEventQueue>>,
        clock: &SimClock,
    ) -> Result<(), SCurveError> {
        let trajectory = self.generate_s_curve(distance, start_velocity, endVelocity, cruise_velocity)?;
        for point in trajectory {
            let event = SimEvent {
                timestamp: clock.current_time + Duration::from_secs_f64(point.time),
                event_type: SimEventType::Step,
                payload: Some(Box::new(StepCommand {
                    axis,
